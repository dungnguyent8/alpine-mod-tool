  ...
  , Zm = Re({
    ...
    setup(e) {
        const t = Bt()
          ...
          , m = X(null);
        ut( () => {
            he.devtools.inspectedWindow.eval("window.location.href", (U, V) => {
                !V && typeof U == "string" && (o.value = U)
            }
            );
            const O = localStorage.getItem("bugReportEmail");
            O && (s.value = O)
        }
        );
        const y = O => {
            O && (l.value = [...l.value, ...Array.from(O)])
        }
          , v = () => {
            l.value = [],
          ...
          , A = O => {
            c.value = !1,
            O.dataTransfer?.files && (y(O.dataTransfer.files),
            d.value && (d.value.value = ""))
        }
          , k = () => {
            d.value?.click()
          ...
          , $ = async () => {
            ...
            if (O.append("email", s.value),
            O.append("url", o.value),
            O.append("feedback", i.value),
            O.append("store", Ii({
                ui: t.$state,
                settings: n.$state,
                components: r.$state
            ...
            l.value.length > 0)
                for (const U of l.value)
                    O.append("files", U);
            try {
                localStorage.setItem("bugReportEmail", s.value),
                await Se.post("/api/v1/bug-report", O, {
                    baseURL: "https://alpinejs.pro",
                    headers: {
                        "Content-Type": "multipart/form-data"
                    }
                }),
                ...
                console.error("Error submitting bug report:", U),
                u.value = "error"
            } finally {
                a.value = !1
            }
        ...
        N("div", zm, [b("form", {
            ...
            class: "text-lg font-semibold text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Report a bug / Provide feedback ", -1)), b("div", null, [U[3] || (U[3] = b("label", {
            for: "email",
            class: "block text-sm font-medium text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Email (optional) ", -1)), it(b("input", {
            type: "email",
            id: "email",
            "onUpdate:modelValue": U[0] || (U[0] = V => s.value = V),
            class: "mt-1 block w-full rounded border border-devtools-outline bg-devtools-surface px-3 py-2 text-devtools-text-primary dark:border-devtools-outline-dark dark:bg-devtools-surface-dark dark:text-devtools-text-primary-dark sm:text-sm",
            placeholder: "work@email.com"
        }, null, 512), [[Qt, s.value]]), U[4] || (U[4] = b("p", {
            class: "mt-1 text-xs text-devtools-text-secondary dark:text-devtools-text-secondary-dark"
        }, " If you want to get in touch and be notified when the issue is fixed. ", -1))]), b("div", null, [U[5] || (U[5] = b("label", {
            for: "url",
            class: "block text-sm font-medium text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Website URL (optional) ", -1)), it(b("input", {
            type: "text",
            id: "url",
            "onUpdate:modelValue": U[1] || (U[1] = V => o.value = V),
            class: "mt-1 block w-full rounded border border-devtools-outline bg-devtools-surface px-3 py-2 text-devtools-text-primary dark:border-devtools-outline-dark dark:bg-devtools-surface-dark dark:text-devtools-text-primary-dark sm:text-sm",
            placeholder: "https://example.com"
        }, null, 512), [[Qt, o.value]]), U[6] || (U[6] = b("p", {
            class: "mt-1 text-xs text-devtools-text-secondary dark:text-devtools-text-secondary-dark"
        }, " URL of the page where you saw the bug. ", -1))]), b("div", null, [U[7] || (U[7] = b("label", {
            for: "feedback",
            class: "block text-sm font-medium text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Bug description / Feedback ", -1)), it(b("textarea", {
            id: "feedback",
            "onUpdate:modelValue": U[2] || (U[2] = V => i.value = V),
            rows: "4",
            required: "",
            class: "mt-1 block w-full rounded border border-devtools-outline bg-devtools-surface px-3 py-2 text-devtools-text-primary dark:border-devtools-outline-dark dark:bg-devtools-surface-dark dark:text-devtools-text-primary-dark sm:text-sm",
            placeholder: "Describe the problem you're facing..."
        }, null, 512), [[Qt, i.value]])]), b("div", null, [U[11] || (U[11] = b("label", {
            class: "mb-1 block text-sm font-medium text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Attach files (optional) ", -1)), b("div", {
            onClick: k,
            onDragenter: tt(x, ["prevent", "stop"]),
            onDragover: tt(x, ["prevent", "stop"]),
            onDragleave: tt(S, ["prevent", "stop"]),
            onDrop: tt(A, ["prevent", "stop"]),
            class: de(["mt-1 flex cursor-pointer justify-center rounded border-2 border-dashed px-6 pb-6 pt-5", c.value ? "border-devtools-primary bg-devtools-primary-container/20 dark:border-devtools-primary-dark dark:bg-devtools-primary-container-dark/20" : "border-devtools-outline hover:border-devtools-primary/70 dark:border-devtools-outline-dark dark:hover:border-devtools-primary-dark/70"])
        }, [b("div", qm, [U[8] || (U[8] = b("svg", {
            class: "mx-auto h-12 w-12 text-devtools-text-secondary dark:text-devtools-text-secondary-dark",
            stroke: "currentColor",
            fill: "none",
            viewBox: "0 0 48 48",
            "aria-hidden": "true"
        ...
        })], -1)), b("div", Km, [b("p", Wm, re(c.value ? "Drop files here" : "Drag and drop files here, or click to select"), 1)]), U[9] || (U[9] = b("p", {
            class: "text-xs text-devtools-text-disabled dark:text-devtools-text-disabled-dark"
        }, " PNG, JPG, GIF, LOG, TXT, etc. ", -1))])], 34), b("input", {
            ...
            onChange: f,
            class: "hidden"
        }, null, 544), l.value && l.value.length > 0 ? (C(),
        N("div", Gm, [U[10] || (U[10] = b("p", {
            class: "text-sm font-medium text-devtools-text-primary dark:text-devtools-text-primary-dark"
        }, " Selected files: ", -1)), b("ul", Jm, [(C(!0),
        N(ce, null, Oe(l.value, V => (C(),
        N("li", {
            key: V.name
        }, re(V.name), 1))), 128))]), b("button", {
            type: "button",
            onClick: v,
            class: "mt-1 text-xs text-devtools-error hover:underline dark:text-devtools-error-dark"
        }, " Clear selection ")])) : ae("", !0), !l.value || l.value.length === 0 ? (C(),
        N("p", Ym, " Attach screenshots or other relevant files. ")) : ae("", !0)]), b("div", Xm, [b("button", {
            type: "submit",
            disabled: a.value || !i.value,
            class: "inline-flex w-full justify-center rounded border border-devtools-primary bg-devtools-primary-container px-4 py-2 text-sm font-medium text-devtools-primary hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50 dark:border-devtools-primary-dark dark:bg-devtools-primary-container-dark dark:text-devtools-primary-dark"
        }, re(a.value ? "Submitting..." : "Submit Report"), 9, Qm), u.value === "success" ? (C(),
        N("p", {
            key: 0,
            ref_key: "successMessageRef",
            ref: p,
            class: "w-full rounded border border-green-500 p-2 text-center text-sm text-green-600 dark:border-green-400 dark:text-green-400"
        }, " Report submitted successfully! ", 512)) : ae("", !0), u.value === "error" ? (C(),
        N("p", {
            key: 1,
            ref_key: "errorMessageRef",
            ref: m,
            class: "w-full rounded border border-red-500 p-2 text-center text-sm text-red-600 dark:border-red-400 dark:text-red-400"
        }, " Failed to submit report. Please try again. ", 512)) : ae("", !0)])], 32)]))
    }
})
  , eg = {
